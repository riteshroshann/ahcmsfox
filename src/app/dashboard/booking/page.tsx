...
export default function BookingPage() {
  ...
  const findMatches = useCallback(() => {
    ...
    startTransition(async () => {
      ...
      const { getRoomsForHostel } = await import("@/app/actions/rooms");
      const roomsData = await getRoomsForHostel(profile.hostel_code);
      if (!roomsData) return;

      // Filter available rooms
      const rooms = roomsData.filter((r: any) => r.current_occupancy < r.capacity).slice(0, 20);
